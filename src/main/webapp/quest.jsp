...
<h1>SPACEQUEST</h1>
    ...
    <p class="lead text-warning col-md-6">${sessionScope.quest.getText()}</p>
<div class="text-warning">
    <c:if test="${sessionScope.stage==1}">
        <p class="lead">Ваши приказания?</p>
        <form action="/controller-servlet" method="GET">
            <c:forEach var="answer" items="${sessionScope.quest.getAnswers()}">
            <input class="form-check-input" type="radio" id="this" name="chosen" checked value="${answer.idQuest}">
            <label class="form-check-label" for="this"><p class="lead"> ${answer.text}</p></label><br>
            </c:forEach>
            <button type="submit" class="btn btn-outline-warning">Подтвердить</button>
        </form>
    </c:if>
<c:if test="${sessionScope.stage==0}">
    <p class="lead col-md-6">${sessionScope.quest.getAnswers().get(0).getText()}</p>
    <form action="/controller-servlet" method="GET">
...
<div class="position-fixed text-success p-3 border border-success rounded bg-black ">
    ...
    <p class="mb-1">Колличество игр: ${sessionScope.questCounter}</p>
